    ...
    public partial class PhongKS : Form
    {
        ...
        public void getDSPLSKH()
        {
            ...
            foreach (CHistory lskh in frmmng.Data.ArrLS)
            {
                ...
                {
                    if(lskh.Ctmbill.Dp.Phong.Count==1)
                    {
                        CPhong a = timSP(lskh.Ctmbill.Dp.Phong[0].Sophong);
                        if (a == null)
                            dsplskh.Add(lskh.Ctmbill.Dp.Phong[0].Sophong, lskh.Ctmbill.Dp.Phong[0]);
                    }
                    else
                    {
                        for(int j = 0;j<lskh.Ctmbill.Dp.Phong.Count;j++)
                        {
                            CPhong a = timSP(lskh.Ctmbill.Dp.Phong[j].Sophong);
                            if (a == null)
                                dsplskh.Add(lskh.Ctmbill.Dp.Phong[j].Sophong, lskh.Ctmbill.Dp.Phong[j]);
                        }
                    }
                }
            }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            if (frmmng.Data.ArrPKS.Count > 0)
            {
                ...
                if(a!=null)
                {
                    MessageBox.Show("Phòng này tồn tại ở lskh");
                    return;
                }
                if (string.Equals(lvwPKS.Items[i].SubItems[2].Text, "Booked"))
                {
                    MessageBox.Show("Phòng đang có khách hàng ở.");
                    return;
                }
